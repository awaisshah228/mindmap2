  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: "No API key. Add your key in Settings → Integration to generate Draw.io diagrams." },
        { status: 400 }
      );
    }
    ...
    const readable = new ReadableStream<Uint8Array>({
      async start(controller) {
        try {
          ...
          console.error("Generate Draw.io stream error:", err);
    ...
    console.error("Generate Draw.io error:", err);
    return NextResponse.json(
